...
class Acromidia_Manager
{
    ...
    public function register_admin_menu()
    {
        ...
        add_action('admin_menu', function () {
            if (Acromidia_Settings::get('restrict_admin') !== 'yes')
                return;
            ...
            $remove_pages = [
                ...
                'tools.php',
                'options-general.php',
                'edit.php?post_type=acro_client',
                'edit.php?post_type=acro_log',
                'edit.php?post_type=acro_transaction',
                'edit.php?post_type=acro_document',
                'edit.php?post_type=acro_task',
                'separator1',
                'separator2',
                'separator-last'
            ];
    ...
    public function import_asaas_clients(\WP_REST_Request $request)
    {
        ...
        foreach ($existing as $p) {
            ...
            if ($a_id)
                $existing_asaas_ids[] = $a_id;

            $cpf = get_post_meta($p->ID, '_acro_cpf_cnpj', true);
            if ($cpf)
                $existing_cpfs[] = preg_replace('/\D/', '', $cpf);
        }
    ...
    {
        if (!Acromidia_Gateway_Factory::is_configured() || !Acromidia_Settings::has('wa_token'))
            return;
    ...
    public function toggle_client_block(\WP_REST_Request $request)
    {
        ...
        if (!$client || $client->post_type !== 'acro_client')
    ...
    public function create_transaction(\WP_REST_Request $request)
    {
        ...
        if (is_wp_error($p_id))
            return new \WP_REST_Response(['error' => 'Falha ao criar'], 500);
    ...
    public function update_transaction(\WP_REST_Request $request)
    {
        ...
        if (isset($params['amount']))
            update_post_meta($id, '_acro_amount', floatval($params['amount']));
        if (isset($params['type']))
            update_post_meta($id, '_acro_type', sanitize_text_field($params['type']));
        if (isset($params['category']))
            update_post_meta($id, '_acro_category', sanitize_text_field($params['category']));
        if (isset($params['date']))
            update_post_meta($id, '_acro_date', sanitize_text_field($params['date']));
        if (isset($params['status']))
            update_post_meta($id, '_acro_status', sanitize_text_field($params['status']));
        if (isset($params['recurring']))
            update_post_meta($id, '_acro_recurring', !empty($params['recurring']) ? 1 : 0);
    ...
    public function update_document(\WP_REST_Request $request)
    {
        ...
        if (isset($params['type']))
            update_post_meta($id, '_acro_doc_type', sanitize_text_field($params['type']));
        if (isset($params['client_id']))
            update_post_meta($id, '_acro_client_id', sanitize_text_field($params['client_id']));
        if (isset($params['client_name']))
            update_post_meta($id, '_acro_client_name', sanitize_text_field($params['client_name']));
        if (isset($params['client_email']))
            update_post_meta($id, '_acro_client_email', sanitize_text_field($params['client_email']));
        if (isset($params['items']))
            update_post_meta($id, '_acro_items', $params['items']);
        if (isset($params['total']))
            update_post_meta($id, '_acro_total', floatval($params['total']));
        if (isset($params['terms']))
            update_post_meta($id, '_acro_terms', wp_kses_post($params['terms']));
        if (isset($params['content']))
            update_post_meta($id, '_acro_doc_content', wp_kses_post($params['content']));
        if (isset($params['contact_name']))
            update_post_meta($id, '_acro_doc_contact', sanitize_text_field($params['contact_name']));
    ...
    public function create_task(\WP_REST_Request $request)
    {
        ...
        if (is_wp_error($p_id))
            return new \WP_REST_Response(['error' => 'Falha'], 500);
    ...
    public function update_task(\WP_REST_Request $request)
    {
        ...
        if (isset($params['status']))
            update_post_meta($id, '_acro_task_status', sanitize_text_field($params['status']));
        if (isset($params['priority']))
            update_post_meta($id, '_acro_task_priority', sanitize_text_field($params['priority']));
        if (isset($params['client_id']))
            update_post_meta($id, '_acro_client_id', sanitize_text_field($params['client_id']));
        if (isset($params['client_name']))
            update_post_meta($id, '_acro_client_name', sanitize_text_field($params['client_name']));
        if (isset($params['document_id']))
            update_post_meta($id, '_acro_document_id', sanitize_text_field($params['document_id']));
